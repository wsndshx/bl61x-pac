#[doc = "Register `rtc_time_lo` reader"]
pub type R = crate::R<RTC_TIME_LO_SPEC>;
#[doc = "Register `rtc_time_lo` writer"]
pub type W = crate::W<RTC_TIME_LO_SPEC>;
#[doc = "Field `rtc_time_latch_l` reader - "]
pub type RTC_TIME_LATCH_L_R = crate::FieldReader<u32>;
#[doc = "Field `rtc_time_latch_l` writer - "]
pub type RTC_TIME_LATCH_L_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    #[doc = "Bits 0:31"]
    #[inline(always)]
    pub fn rtc_time_latch_l(&self) -> RTC_TIME_LATCH_L_R {
        RTC_TIME_LATCH_L_R::new(self.bits)
    }
}
impl W {
    #[doc = "Bits 0:31"]
    #[inline(always)]
    #[must_use]
    pub fn rtc_time_latch_l(&mut self) -> RTC_TIME_LATCH_L_W<RTC_TIME_LO_SPEC> {
        RTC_TIME_LATCH_L_W::new(self, 0)
    }
    #[doc = r" Writes raw bits to the register."]
    #[doc = r""]
    #[doc = r" # Safety"]
    #[doc = r""]
    #[doc = r" Passing incorrect value can cause undefined behaviour. See reference manual"]
    #[inline(always)]
    pub unsafe fn bits(&mut self, bits: u32) -> &mut Self {
        self.bits = bits;
        self
    }
}
#[doc = "Low bits of Real-Time Clock time\n\nYou can [`read`](crate::generic::Reg::read) this register and get [`rtc_time_lo::R`](R).  You can [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`write_with_zero`](crate::generic::Reg::write_with_zero) this register using [`rtc_time_lo::W`](W). You can also [`modify`](crate::generic::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct RTC_TIME_LO_SPEC;
impl crate::RegisterSpec for RTC_TIME_LO_SPEC {
    type Ux = u32;
}
#[doc = "`read()` method returns [`rtc_time_lo::R`](R) reader structure"]
impl crate::Readable for RTC_TIME_LO_SPEC {}
#[doc = "`write(|w| ..)` method takes [`rtc_time_lo::W`](W) writer structure"]
impl crate::Writable for RTC_TIME_LO_SPEC {
    const ZERO_TO_MODIFY_FIELDS_BITMAP: Self::Ux = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: Self::Ux = 0;
}
#[doc = "`reset()` method sets rtc_time_lo to value 0"]
impl crate::Resettable for RTC_TIME_LO_SPEC {
    const RESET_VALUE: Self::Ux = 0;
}
